...
resource "null_resource" "name" {
  ...
  connection {
    type        = "ssh"
    user        = "ec2-user"
    private_key = file("~/Downloads/somuwordpres.pem")
  ...
  provisioner "file" {
    source      = "~/Downloads/terraform/wordpress/docker-tutorial-main/Dockerfile.txt"
    destination = "/home/ec2-user/Dockerfile"
  }

  # copy the build_docker_image.sh from your computer to the ec2 instance 
  provisioner "file" {
    source      = "~/Downloads/terraform/wordpress/docker-tutorial-main/build_docker_image.sh"
